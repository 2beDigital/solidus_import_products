...
module SolidusImportProducts
  module Parser
    class Csv < Base
      DEFAULT_CSV_ENCODING = %w[iso-8859-1 UTF-8].freeze
      ...
      def initialize(data_file, options)
        ...
        encoding_csv = (options[:encoding_csv] if options) || DEFAULT_CSV_ENCODING.first
        separator_char = (options[:separator_char] if options) || DEFAULT_CSV_SEPARATOR
        csv_string = open(data_file, "r:#{encoding_csv}").read.encode(DEFAULT_CSV_ENCODING.last)
